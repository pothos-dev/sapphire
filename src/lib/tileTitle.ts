// Per-Pane header title derivation (pure; no DOM/IPC/runes).
//
// A Pane's header shows a compact label for the active Concept. The label
...
// helper so the PaneHeader component stays thin and the rule is unit-testable.

import { basename, stripMd } from '$lib/path';
import type { Property } from '$lib/frontmatter';

/**
 * Derive the header label for the Pane showing `path` with `properties`
 ...
 */
export function paneTitle(path: string | null, properties: Property[]): string {
  if (path === null) return '';
