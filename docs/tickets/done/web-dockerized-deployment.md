...
Package Sunstone Web as a Docker image for internal hosting: one container serves
...
- Multi-stage Dockerfile: build the frontend (`vite build` / SvelteKit adapter-node)
  and compile `sunstone-server` (`cargo build --release`) in build stages; the final
...
- The Bundle is provided as a read-only volume mount, its path passed via
  `SUNSTONE_BUNDLE`. The container reads the Bundle but never writes to it.
...
- [ ] Multi-stage Dockerfile builds frontend + `sunstone-server` and produces a runnable image
- [ ] `docker compose up` serves the SSR web viewer against a bundle mounted via `SUNSTONE_BUNDLE`
