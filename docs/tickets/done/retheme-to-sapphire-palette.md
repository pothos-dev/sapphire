...
Replace the app's jade/emerald **green** identity with a **sapphire-blue** palette that matches the new app icon, so the UI's colour identity tells the same story as the brand.
...
- **Accent** — the current emerald/jade accent (`--accent`, `--accent-contrast`, `--accent-soft`, `--accent-ring`, and the derived tag-chip tokens) re-cast as sapphire blue, preserving the same muted/sophisticated feel (not a loud primary blue).
...
Keep the existing token structure and naming; only the values change. Update the "palette identity" comment block in `app.css` to describe the sapphire identity.

## Acceptance criteria

- [ ] Light and dark modes both render a cohesive sapphire-blue accent over cool neutral backgrounds, matching the icon
